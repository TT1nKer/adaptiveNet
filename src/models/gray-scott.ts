...
— 循环六个预设（mitosis、worms、maze、ξ-spirals、β-wavefield、α-chaos、U-skate）。
— 实时拖动 (f, k) 遍历 Pearson 图谱。
— α 区域里同 seed 同轨迹，不同 seed 发散——伪装的正 Lyapunov 指数。
...
— Cycle through the six presets (mitosis, worms, maze, ξ-spirals, β-wavefield, α-chaos, U-skate).
— Drag (f, k) live to walk the Pearson map.
— In the α region, identical seed gives identical trajectory; different seeds diverge — a positive Lyapunov exponent in disguise.
