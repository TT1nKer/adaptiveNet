// Graph generators returning { N, adj, edges, deg } in a uniform shape.
// Sparse representation: adj[i] is a JS array of neighbor indices.

function emptyAdj(n) { return Array.from({ length: n }, () => []); }

function addEdge(adj, edges, i, j) {
  if (i === j) return false;
...
function finalize(N, adj, edges) {
  const deg = new Int32Array(N);
  for (let i = 0; i < N; i++) deg[i] = adj[i].length;
  return { N, adj, edges, deg };
}

export function buildER(N, avgK, rng) {
  const adj = emptyAdj(N), edges = [];
...
export function buildBA(N, avgK, rng) {
  const adj = emptyAdj(N), edges = [];
...
export function buildWS(N, avgK, beta, rng) {
  const adj = emptyAdj(N), edges = [];
