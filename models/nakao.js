...
import { generators } from '../graph.js';
...
Reference: Nakao & Mikhailov, *Nature Physics* 6, 544–550 (2010).`,
  ...
  init(params, rng) {
    ...
    const topo  = params.topo;
    const graph = generators[topo](N, k, rng);
  ...
  render: {
    nodeColor(state, i) {
      const u = state.X[i * 2];
      // diverging map around u* = 5; saturate beyond ±4.5
      let t = (u - 5) / 4.5;
  ...
  observe: {
    ...
    timeSeries: {
      ...
      value(state) {
        const N = state.N;
        let s1 = 0, s2 = 0;
        for (let i = 0; i < N; i++) { const u = state.X[i * 2]; s1 += u; s2 += u * u; }
        const m = s1 / N;
